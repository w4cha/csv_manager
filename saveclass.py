...
class CsvClassSave:
    ...
    def leer_datos_csv(self, search = "", back_up = False, escaped = False) -> Generator[list[str], None, None]:
    ...
    def guardar_datos_csv(self, enforce_unique = None) -> str:
        ...
        if enforce_unique is not None and self.current_rows > 1:
            if not isinstance(enforce_unique, tuple):
                raise ValueError(f"el parámetro enforce_unique debe ser una tuple pero fue {type(enforce_unique).__name__}")
                ...
                raise ValueError(f"la tuple solo debe contener str su tuple contiene {', '.join([str(type(item).__name__) for item in enforce_unique])}")
    ...
    def return_pattern(str_pattern) -> tuple | None:
        ...
        if not isinstance(str_pattern, str):
            raise ValueError(f"debe ingresar un str donde buscar un patron, pero se introdujo {type(str_pattern).__name__}")
